...
このドキュメントは、Paper Tipster の機能要件と仕様をまとめたものです。
ユーザー機能、管理者機能、システムの動き、データの流れをわかりやすく説明します。
...
- **ゲストログイン (簡易登録)**: 管理者が発行した「登録用コード」を使用してアカウントを作成・ログインします。
  - **共有コード**: 1つのコードを複数ユーザー（集まりの参加者など）で共有できます。
  - **識別**: 同じコードを使ったユーザーを同じグループとして扱えます。
  - **フロー**: コード・ユーザー名・絵文字パスワードを入力。
    - **新規**: 入力情報で新しいアカウントを作成します。
    - **既存**: コードとユーザー名が一致する場合、パスワードを確認してログインします。
...
- **お小遣い (Claim)**: イベントに参加すると初期資金（イベント通貨）を受け取れます。
- **取引履歴**: 各イベントウォレット内での資金移動（配布、ベット、払戻、借入）を確認できます。
- **借入（ローン）**:
  - ウォレット残高が配布金額の60%未満になると、借り入れできます。
  - 即BETページおよび馬券購入画面に借入の導線バナーが表示されます。
  - 借入はイベントにつき1回のみ。
  - 借入額はイベントごとに設定できます（未設定時は配布金額と同額）。
  - 借入記録は取引履歴に `LOAN` タイプとして記録されます。
...
- **購入フロー**:
  ...
  5. 金額入力と購入確定
- **即BET (Soku-BET)**:
  - 開催中レースに特化したカードUIで、競馬場・レース番号・条件を見ながらすぐに投票画面へ移動できます。
- **制限事項**:
  - 所持金を超える購入はできません。
  - 受付終了（締め切り）後は購入できません。
...
### 1.5 レース待機・結果確認 (Standby & Results)

- **待機画面**:
  - レース情報の表示（競馬場、レース名、発走予定/締切）。
  - リアルタイムなステータス表示（待機中 / 確定済み）。
  - **着順表示**: 着順が確定すると「着順速報」として表示します。馬番には枠色を反映します。払戻確定後も「確定着順」を維持し、再読み込み後も同じ表示になります。
- **結果発表**:
...
- **表示モード**: 管理者の設定により表示内容が異なります。
  ...
  - **公開（借金込み）**: 完全公開に加え、各ユーザーの借入額が表示されます。
...
- **レースマスタ登録**: レース名、グレード、種別、デフォルトの距離・馬場種類・競馬場・回り方向を登録します。
...
- **ランキング公開設定**:
  ...
  - `FULL_WITH_LOAN`: 公開（借金込み）(完全公開 + 借入額の表示)
- **借入額設定**: イベントごとに借入可能金額を設定できます（空欄時は配布金額と同額）。

### 2.7 オッズ管理 (Odds Management)

- **デフォルト保証オッズ設定**: システム全体で適用される、新規レース作成時のデフォルト保証オッズを管理します。
- **レース別設定**: 各レースの編集画面で、レースごとの保証オッズに上書きできます。

### 2.8 レース (Race Instance) 管理

- **レース作成**: 特定のイベントと競馬場に紐付けてレースインスタンスを作成します。レース定義の値を引き継ぎ、必要な項目だけ上書きできます。
- **基本情報**: 開催日、競馬場、レース番号、レース名、距離、コース（芝/ダート）、馬場状態、発走時刻。
- **出走馬管理 (Race Entry)**: ドラッグ＆ドロップUIで出走馬を割り当て、枠順・馬番を設定します。アコーディオンリストでレース番号（1Rなど）と登録頭数をすぐ確認できます。
- **ステータス遷移**:
  - **出走前 (SCHEDULED)**: レース前、馬券購入可能。
  - **締切済み (CLOSED)**: 馬券購入締切、結果入力待ち。着順入力もこの段階で行う。
  - **払戻確定 (FINALIZED)**: 払戻計算が完了し、ウォレットに残高が反映された状態。
  - **中止 (CANCELLED)**: レースが中止になった状態。

### 2.9 出走管理 (Race Entry)

- **出走馬登録**: Drag & Drop (DnD) UIで登録済み競走馬をレースに割り当てます。
- **枠順確定**: JRAルールなどを参考に枠を割り当てます（現在は手動または簡易ロジック）。
- **レース表示**: アコーディオンリスト形式で、レース番号（1Rなど）や登録頭数を一目で確認できます。

### 2.10 レース進行・着順確定 (Operation)

- **リアルタイム操作**: 受付開始/終了（手動・タイマー）、着順入力、払戻計算、結果配信（SSE）の全行程をワンストップで管理。

1. **受付管理**:

- **キッチンタイマー**: 「+5分」「+10分」などのボタンで、指定時間後に自動締切を設定できます。
...
### 2.11 馬券管理 (Betting Management)

- **一覧確認**: 管理者は、レースごとに全ユーザーの購入馬券（購入内容、金額、ユーザー名）を一覧で確認できます。
- **詳細確認**: 個別の馬券のステータスや的中の有無を確認できます。

### 2.12 クイックガイド (Quick Guide)

- **基本操作ガイド**: 管理画面での「イベント作成 -> レース作成 -> 出走登録 -> BET5設定」の一連の流れをステップ形式で解説します。
- **導線**: 管理者ダッシュボードのトップに配置され、新規管理者や久しぶりに操作する管理者をサポートします。
...
- `src/features`: 機能単位のモジュール (admin, auth, betting, economy, forecasts, ranking, stats, user)
- `src/entities`: データモデルとドメインロジック (bet, horse, race, ranking, user, wallet)
