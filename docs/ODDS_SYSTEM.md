...
- **計算対象**:
  ...
  - `複勝 (PLACE)` は枠組みのみ用意されていますが、計算ロジックは未実装（空オブジェクト）です。
- **計算式**:
  1.  **総投票数 (Pool)**: そのレースの単勝投票総額を集計。
  2.  **控除率 (Deduction)**: 総投票額から20% (`ODDS_DEDUCTION_RATE = 0.2`) を控除。
      - `Return Amount = Pool * (1 - 0.2)`
  3.  **個別オッズ**: 各馬への投票額で割る。
      - `Odds = Return Amount / Vote Amount`
  4.  **端数処理**: 小数点第2位以下を切り捨て（例: 2.46 -> 2.4）。
  5.  **最低保証**: 1.1倍未満にはなりません（JRA準拠）。
  6.  **ゼロ除算対策**: 投票数が0の場合はオッズを `0.0` とします。
...
- **計算式**:
  1. **総ポット (Total Pot)**: `Initial Pot` (キャリーオーバー含む) + `Total Sales` (当回の売上)
  2. **配当 (Dividend)**: `Total Pot / Winner Count` (小数点以下切り捨て)
  3. **特記事項**: 控除率は適用されず（0%）、総取り方式となります。的中者なしの場合は次回へ全額キャリーオーバー（仕様検討中、現状はプール維持）。
