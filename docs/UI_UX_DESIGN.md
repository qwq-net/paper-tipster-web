...
このドキュメントは、アプリケーション設計・実装の指針をまとめたものです。
見た目の良さだけでなく、迷わず操作できる体験を作ることを目的とします。

---

## 1. 情報設計とレイアウト (Information Architecture & Layout)

ユーザーが迷わず、必要な情報をすぐ見つけられる構造を目指します。

### 1.1 視覚的階層 (Visual Hierarchy) & ビジュアル・アンカー (Visual Anchor)

- **原則**: 重要な要素を見つけやすくし、視線の流れを整える。
- **適用**:
...
- **原則**: ユーザーが慣れたUIパターンで、直感的に操作できるようにする。
- **適用**:
...
- **原則**: 一度に表示する情報を絞り、必要な時だけ詳細を見せる。
- **適用**:
  - **馬情報**: 一覧では「馬名」「オッズ」のみ表示し、タップで「過去の戦績」「血統」などの詳細を展開（アコーディオンUIやモーダル）する。
  - **イベント編集**: 一覧性を損なわないよう、編集画面はIntercepting Routesを用いたモーダルで表示し、詳細設定へのアクセスをスムーズにする。
  - **ステップ実行**: 会員登録や馬券購入などの複雑なタスクは、1画面1タスクに分割する。

### 2.2 決断疲れ (Decision Fatigue)

- **原則**: 選択肢が多すぎると、ユーザーは選びにくくなる。
- **適用**:
...
- **原則**: つまずいた時や初回利用時など、必要な場面でだけヘルプを表示する。
- **適用**:
...
ユーザーが迷わず次の操作に進めるようにします。

### 3.1 デフォルト効果 (Default Bias) & おとり効果 (Decoy Effect)

- **原則**: デフォルトの選択肢は選ばれやすい。比較しやすい並びにすると判断しやすくなる。
- **適用**:
...
- **原則**: 最初に見た数字は判断基準になりやすい。伝え方で印象も変わる。
- **適用**:
...
- **原則**: ゴールが見えると行動しやすくなる。未完了タスクは意識に残りやすい。
- **適用**:
...
- 視認性
  - 小さすぎる文字は使わない。最低でも12px以上を確保する。
  - Tailwind CSSでは `text-sm` 以上の文字サイズを使う。
